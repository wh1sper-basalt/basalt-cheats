-- =============================================================================
-- FULL.sql — auto-rebuilt from partial dumps
-- =============================================================================
...
-- Normal cheats / utilities: 1 / 30 / 90 / 365 days + Lifetime (premium slugs: min 7 days)
...
FROM cheats c
JOIN (
  SELECT '1 день' AS label_ru, '1 day' AS label_en, 1 AS duration_days, 0 AS sort_order
  UNION ALL SELECT '30 дней', '30 days', 30, 1
...
WHERE (c.os_id IS NULL OR c.os_id = 1)
  AND c.slug NOT IN (
    'basalt-ultra', 'raid-ultra', 'gta-pro', 'wt-pro', 'genshin-pro', 'spoofer-pro', 'pubg-omega',
    'stealth-raid', 'edge-raid', 'eft-zenith', 'eft-axis', 'rust-ultra', 'fn-proxima'
  );
...
FROM cheats c
JOIN (
  SELECT '7 дней' AS label_ru, '7 days' AS label_en, 7 AS duration_days, 0 AS sort_order
  UNION ALL SELECT '30 дней', '30 days', 30, 1
  UNION ALL SELECT '90 дней', '90 days', 90, 2
  UNION ALL SELECT '365 дней', '365 days', 365, 3
  UNION ALL SELECT 'Навсегда', 'Lifetime', 0, 4
...
-- Album slides: WebP 16:9 (paths relative to site/) — cover uses -001 in cheats.image_path
INSERT INTO `cheat_media` (`cheat_id`, `image_path`, `caption_ru`, `caption_en`, `sort_order`)
SELECT c.id, CONCAT('assets/img/cheats/albums/', c.slug, '-001.webp'), '', '', 1 FROM `cheats` c
UNION ALL
SELECT c.id, CONCAT('assets/img/cheats/albums/', c.slug, '-002.webp'), '', '', 2 FROM `cheats` c
UNION ALL
SELECT c.id, CONCAT('assets/img/cheats/albums/', c.slug, '-003.webp'), '', '', 3 FROM `cheats` c;

UPDATE `cheats` SET `image_path` = CONCAT('assets/img/cheats/albums/', `slug`, '-001.webp');
...
INSERT INTO `key_plans` (`cheat_id`, `label_ru`, `label_en`, `duration_days`, `price_rub`, `sort_order`)
SELECT c.id, '1 день', '1 day', 1, 590, 0
FROM `cheats` c
LEFT JOIN `key_plans` p ON p.cheat_id = c.id AND p.duration_days = 1
WHERE c.id > 26 AND (c.os_id IS NULL OR c.os_id = 1) AND p.id IS NULL
  AND c.slug NOT IN (
    'basalt-ultra', 'raid-ultra', 'gta-pro', 'wt-pro', 'genshin-pro', 'spoofer-pro', 'pubg-omega',
    'stealth-raid', 'edge-raid', 'eft-zenith', 'eft-axis', 'rust-ultra', 'fn-proxima'
  );
...
INSERT INTO `key_plans` (`cheat_id`, `label_ru`, `label_en`, `duration_days`, `price_rub`, `sort_order`)
SELECT c.id, '7 дней', '7 days', 7, 990, 0
FROM `cheats` c
LEFT JOIN `key_plans` p ON p.cheat_id = c.id AND p.duration_days = 7
WHERE c.id > 26 AND (c.os_id IS NULL OR c.os_id = 1) AND p.id IS NULL
  AND c.slug IN (
    'basalt-ultra', 'raid-ultra', 'gta-pro', 'wt-pro', 'genshin-pro', 'spoofer-pro', 'pubg-omega',
    'stealth-raid', 'edge-raid', 'eft-zenith', 'eft-axis', 'rust-ultra', 'fn-proxima'
...
FROM `cheats` c
LEFT JOIN `key_plans` p ON p.cheat_id = c.id AND p.sort_order = 1 AND p.duration_days = 30
WHERE c.id > 26 AND (c.os_id IS NULL OR c.os_id = 1) AND p.id IS NULL;

INSERT INTO `key_plans` (`cheat_id`, `label_ru`, `label_en`, `duration_days`, `price_rub`, `sort_order`)
SELECT c.id, '90 дней', '90 days', 90, 4490, 2
FROM `cheats` c
LEFT JOIN `key_plans` p ON p.cheat_id = c.id AND p.sort_order = 2 AND p.duration_days = 90
WHERE c.id > 26 AND (c.os_id IS NULL OR c.os_id = 1) AND p.id IS NULL;

INSERT INTO `key_plans` (`cheat_id`, `label_ru`, `label_en`, `duration_days`, `price_rub`, `sort_order`)
SELECT c.id, '365 дней', '365 days', 365, 9990, 3
FROM `cheats` c
LEFT JOIN `key_plans` p ON p.cheat_id = c.id AND p.sort_order = 3 AND p.duration_days = 365
WHERE c.id > 26 AND (c.os_id IS NULL OR c.os_id = 1) AND p.id IS NULL;

INSERT INTO `key_plans` (`cheat_id`, `label_ru`, `label_en`, `duration_days`, `price_rub`, `sort_order`)
SELECT c.id, 'Навсегда', 'Lifetime', 0, 14990, 4
FROM `cheats` c
LEFT JOIN `key_plans` p ON p.cheat_id = c.id AND p.duration_days = 0
WHERE c.id > 26 AND (c.os_id IS NULL OR c.os_id = 1) AND p.id IS NULL;
...
SET kp.price_rub = CASE
  ...
  WHEN kp.duration_days = 1 THEN 590
  WHEN kp.duration_days = 7 THEN 990
...
-- Р’СЂРµРјСЏ СЃРѕР·РґР°РЅРёСЏ: Р”РµРє 18 2023 Рі., 13:58
-- Р’РµСЂСЃРёСЏ СЃРµСЂРІРµСЂР°: 5.7.39
-- Р’РµСЂСЃРёСЏ PHP: 8.1.9
...
--
-- Р‘Р°Р·Р° РґР°РЅРЅС‹С…: `db_sweetty`
--
...
--
-- РЎС‚СЂСѓРєС‚СѓСЂР° С‚Р°Р±Р»РёС†С‹ `cakes`
--
...
--
-- РћС‡РёСЃС‚РёС‚СЊ С‚Р°Р±Р»РёС†Сѓ РїРµСЂРµРґ РґРѕР±Р°РІР»РµРЅРёРµРј РґР°РЅРЅС‹С… `cakes`
--

TRUNCATE TABLE `cakes`;
--
-- Р”Р°РјРї РґР°РЅРЅС‹С… С‚Р°Р±Р»РёС†С‹ `cakes`
--

INSERT INTO `cakes` (`id_cake`, `title`, `information`, `path`) VALUES
(1, 'РќР•Р–РќР«Р™ Р‘РђР РҐРђРў', 'Р‘РёСЃРєРІРёС‚РЅС‹Р№ С‚РѕСЂС‚ СЂСѓС‡РЅРѕР№ СЂР°Р±РѕС‚С‹ СЃРѕ СЃР»РёРІРѕС‡РЅС‹Рј РєСЂРµРјРѕРј. РќРµСЃРјРѕС‚СЂСЏ РЅР° РїСЂРѕСЃС‚РѕС‚Сѓ, РёРјРµРµС‚ РѕС‡РµРЅСЊ РЅР°СЂСЏРґРЅС‹Р№ РІРЅРµС€РЅРёР№ РІРёРґ. РРґРµР°Р»СЊРЅРѕ РїРѕРґС…РѕРґРёС‚ РґР»СЏ СЃРІР°РґСЊР±С‹ РёР»Рё Р”РЅСЏ РЎРІСЏС‚РѕРіРѕ Р’Р°Р»РµРЅС‚РёРЅР°.', 'assets/img/cakes/gentle-velvet.jpg'),
(2, 'РР—РЈРњР РЈР”', 'РћС‡РµРЅСЊ СЏСЂРєРёР№ РІРѕ РІРєСѓСЃРµ Рё С†РІРµС‚Рµ С‚РѕСЂС‚! Р—Р° СЃС‡РµС‚ РєРѕРЅС‚СЂР°СЃС‚Р° РєСЂР°СЃРЅРѕРіРѕ Рё Р·РµР»РµРЅРѕРіРѕ РІС‹РіР»СЏРґРёС‚ РїСЂР°Р·РґРЅРёС‡РЅРѕ Рё РїРѕРёСЃС‚РёРЅРµ РєР°Рє РґСЂР°РіРѕС†РµРЅРЅС‹Р№ РєР°РјРµРЅСЊ! РџСЂРµРєСЂР°СЃРЅС‹Р№ РїРѕРґР°СЂРѕРє!', 'assets/img/cakes/emerald.jpg'),
(3, 'РџРўРР§Р¬Р• РњРћР›РћРљРћ', 'РљР»Р°СЃСЃРёРєР° РєРѕРЅРґРёС‚РµСЂСЃРєРѕРіРѕ РёСЃРєСѓСЃСЃС‚РІР° СЃРѕРІРµС‚СЃРєРёС…, РґР° Рё СЂРѕСЃСЃРёР№СЃРєРёС… РІСЂРµРјРµРЅ. Р’РѕР·РґСѓС€РЅС‹Р№, Р»РµРіРєРёР№ Рё Р±РµР»РѕСЃРЅРµР¶РЅС‹Р№!', 'assets/img/cakes/birds-milk.jpg');

-- --------------------------------------------------------

--
-- РЎС‚СЂСѓРєС‚СѓСЂР° С‚Р°Р±Р»РёС†С‹ `feedback`
--
...
--
-- РћС‡РёСЃС‚РёС‚СЊ С‚Р°Р±Р»РёС†Сѓ РїРµСЂРµРґ РґРѕР±Р°РІР»РµРЅРёРµРј РґР°РЅРЅС‹С… `feedback`
--
...
--
-- РЎС‚СЂСѓРєС‚СѓСЂР° С‚Р°Р±Р»РёС†С‹ `fillings`
--
...
--
-- РћС‡РёСЃС‚РёС‚СЊ С‚Р°Р±Р»РёС†Сѓ РїРµСЂРµРґ РґРѕР±Р°РІР»РµРЅРёРµРј РґР°РЅРЅС‹С… `fillings`
--

TRUNCATE TABLE `fillings`;
--
-- Р”Р°РјРї РґР°РЅРЅС‹С… С‚Р°Р±Р»РёС†С‹ `fillings`
--

INSERT INTO `fillings` (`id_filling`, `name`, `path`) VALUES
(1, 'РЇР“РћР”РќР«Р™', 'assets/img/fillings/berry.jpg'),
(2, 'Р¤РРЎРўРђРЁРљРћР’Р«Р™', 'assets/img/fillings/pistachio.jpg'),
(3, 'РљРћРљРћРЎРћР’Р«Р™', 'assets/img/fillings/coconut.jpg'),
(4, 'Р›РРњРћРќРќР«Р™', 'assets/img/fillings/citric.jpg'),
(5, 'РЁРћРљРћР›РђР”РќР«Р™', 'assets/img/fillings/chocolate.jpg'),
(6, 'РњРЇРўРќР«Р™', 'assets/img/fillings/mint.jpg'),
(7, 'РњР•Р”РћР’Р«Р™', 'assets/img/fillings/honey.jpg'),
(8, 'Р¤Р РЈРљРўРћР’Р«Р™', 'assets/img/fillings/fruit.jpg'),
(9, 'РћР Р•РҐРћР’Р«Р™', 'assets/img/fillings/walnut.jpg'),
(10, 'РљРћР¤Р•Р™РќР«Р™', 'assets/img/fillings/coffee.jpg'),
(11, 'РљРђР РђРњР•Р›Р¬РќР«Р™', 'assets/img/fillings/caramel.jpg'),
(12, 'Р™РћР“РЈР РўРћР’Р«Р™', 'assets/img/fillings/yoghurt.jpg');

-- --------------------------------------------------------

--
-- РЎС‚СЂСѓРєС‚СѓСЂР° С‚Р°Р±Р»РёС†С‹ `index_page`
--
...
--
-- РћС‡РёСЃС‚РёС‚СЊ С‚Р°Р±Р»РёС†Сѓ РїРµСЂРµРґ РґРѕР±Р°РІР»РµРЅРёРµРј РґР°РЅРЅС‹С… `index_page`
--

TRUNCATE TABLE `index_page`;
--
-- Р”Р°РјРї РґР°РЅРЅС‹С… С‚Р°Р±Р»РёС†С‹ `index_page`
--

INSERT INTO `index_page` (`id_element`, `alias`, `content`) VALUES
(1, 'banner', '<section class=\"banner first\">\r\n    <div class=\"content\">\r\n      <h1>Р”РћРњРђРЁРќРЇРЇ РљРћРќР”РРўР•Р РЎРљРђРЇ</h1>\r\n      <h2>РЎР”Р•Р›РђРќРћ РЎ Р›Р®Р‘РћР’Р¬Р®...</h2>\r\n    </div>\r\n  </section>'),
(2, 'advantages', '<section class=\"advantages\">\r\n    <div class=\"content\">\r\n      <div class=\"advantage\">\r\n        <img src=\"assets/img/icons/icon_1.png\" alt=\"РќР°С‚СѓСЂР°Р»СЊРЅС‹Рµ РїСЂРѕРґСѓРєС‚С‹\">\r\n        <p> РўРѕР»СЊРєРѕ<br> РЅР°С‚СѓСЂР°Р»СЊРЅС‹Рµ РїСЂРѕРґСѓРєС‚С‹ </p>\r\n      </div>\r\n      <div class=\"advantage\">\r\n        <img src=\"assets/img/icons/icon_2.png\" alt=\"Р‘РµР· РєРѕРЅСЃРµСЂРІР°РЅС‚РѕРІ\">\r\n        <p> Р‘РµР· РєРѕРЅСЃРµСЂРІР°РЅС‚РѕРІ,<br> СЃСЂРѕРє С…СЂР°РЅРµРЅРёСЏ 48 С‡Р°СЃРѕРІ </p>\r\n      </div>\r\n      <div class=\"advantage\">\r\n        <img src=\"assets/img/icons/icon_3.png\" alt=\"РРЅРґРёРІРёРґСѓР°Р»СЊРЅС‹Р№ РґРёР·Р°Р№РЅ\">\r\n        <p> РРЅРґРёРІРёРґСѓР°Р»СЊРЅС‹Р№ РґРёР·Р°Р№РЅ </p>\r\n      </div>\r\n      <div class=\"advantage\">\r\n        <img src=\"assets/img/icons/icon_4.png\" alt=\"Р Р°Р·РЅС‹Рµ РІРєСѓСЃС‹\">\r\n        <p> Р Р°Р·РЅРѕРѕР±СЂР°Р·РёРµ<br> РІРєСѓСЃРѕРІ РІ РѕРґРЅРѕРј С‚РѕСЂС‚Рµ </p>\r\n      </div>\r\n    </div>\r\n  </section>'),
(3, 'select', '<section class=\"select\">\r\n    <div class=\"content\">\r\n      <h2>Рђ Р§Р•Р“Рћ Р–Р•Р›РђР•РўР• Р’Р«?</h2>\r\n      <div class=\"select__cakes\">\r\n        <div class=\"select__cake\">\r\n          <img src=\"assets/img/select-cake-1.jpg\" alt=\"Р’С‹Р±СЂР°С‚СЊ РіРѕС‚РѕРІС‹Р№ С‚РѕСЂС‚\">\r\n          <a href=\"cakes.php\">Р’Р«Р‘Р РђРўР¬ Р“РћРўРћР’Р«Р™</a>\r\n        </div>\r\n        <div class=\"select__cake\">\r\n          <img src=\"assets/img/select-cake-2.jpg\" alt=\"Р—Р°РєР°Р·Р°С‚СЊ СЃРІРѕР№ С‚РѕСЂС‚\">\r\n          <a href=\"index.php#feedback\">РЎРћР—Р”РђРўР¬ РЎР’РћР™</a>\r\n        </div>\r\n      </div>\r\n    </div>\r\n  </section>'),
(4, 'adout', '<section class=\"about\">\r\n    <div class=\"about__wrap\">\r\n      <div class=\"about__text\">\r\n        <p>\r\n          РњРµРЅСЏ Р·РѕРІСѓС‚ Р•РєР°С‚РµСЂРёРЅР° - СЏ РїСЂРѕС„РµСЃСЃРёРѕРЅР°Р»СЊРЅС‹Р№ РєРѕРЅРґРёС‚РµСЂ Рё С…РѕР·СЏР№РєР° РњРѕРµР№\r\n          РЎР»Р°РґРѕСЃС‚Рё.\r\n        </p>\r\n        <p>РњРѕР№ РґРµРІРёР· - С‡РµСЃС‚РЅР°СЏ Рё РѕС‚РєСЂС‹С‚Р°СЏ СЂР°Р±РѕС‚Р°, РёРЅРґРёРІРёРґСѓР°Р»СЊРЅС‹Р№ РїРѕРґС…РѕРґ.</p>\r\n        <p>\r\n          Р’ РјРѕРёС… С‚РѕСЂС‚РёРєР°С… С‚РѕР»СЊРєРѕ РЅР°С‚СѓСЂР°Р»СЊРЅС‹Рµ РёРЅРіСЂРµРґРёРµРЅС‚С‹ Рё РєР°С‡РµСЃС‚РІРµРЅРЅС‹Рµ\r\n          РїСЂРѕРґСѓРєС‚С‹.\r\n        </p>\r\n      </div>\r\n    </div>\r\n  </section>'),
(5, 'reviews', '<section class=\"reviews\">\r\n    <div class=\"content\">\r\n      <div class=\"review\">\r\n        <img src=\"/assets/img/reviews/review-1.jpg\" alt=\"review-1\">\r\n        <div class=\"review__text\">\r\n          <blockquote>\r\n            Р’РЅРёРјР°С‚РµР»СЊРЅРѕРµ РѕС‚РЅРѕС€РµРЅРёРµ Рє РєР»РёРµРЅС‚Сѓ, РІРєСѓСЃРЅСЋС‰РёР№ С‚РѕСЂС‚ Рё РЅРµРєСѓСЃР°С‡РёРµ С†РµРЅС‹! РћРіСЂРѕРјРЅРѕРµ СЃРїР°СЃРёР±Рѕ Р·Р° СѓРґРѕРІРѕР»СЊСЃС‚РІРёРµ! Р•РєР°С‚РµСЂРёРЅРµ С‚РІРѕСЂС‡РµСЃРєРёС… СѓСЃРїРµС…РѕРІ!\r\n          </blockquote>\r\n        </div>\r\n      </div>\r\n      <div class=\"review\">\r\n        <img src=\"/assets/img/reviews/review-2.jpg\" alt=\"review-2\">\r\n        <div class=\"review__text\">\r\n          <blockquote>\r\n            РўРѕСЂС‚ РЅР°С‚СѓСЂР°Р»СЊРЅС‹Р№, РІС‹РїРѕР»РЅРµРЅ РёСЃРєСѓСЃРЅРѕ Рё РїРѕРґР»РѕР¶РєР° Р»РёС€СЊ РїРѕРґС‡РµСЂРєРЅСѓР»Р° С‚РѕСЂР¶РµСЃС‚РІРµРЅРЅРѕРµ Р»Р°РєРѕРјСЃС‚РІРѕ! РЎРїР°СЃРёР±Рѕ Рё СѓСЃРїРµС…РѕРІ РІР°С€РµРјСѓ Р±РёР·РЅРµСЃСѓ!\r\n          </blockquote>\r\n        </div>\r\n      </div>\r\n      <div class=\"review\">\r\n        <img src=\"/assets/img/reviews/review-3.jpg\" alt=\"review-3\">\r\n        <div class=\"review__text\">\r\n          <blockquote>\r\n            РР·С‹СЃРєР°РЅРЅС‹Р№ РІРєСѓСЃ С‚РѕСЂС‚РёРєРѕРІ РЅРµ РѕСЃС‚Р°РІРёР» СЂР°РІРЅРѕРґСѓС€РЅС‹Рј РЅРё РѕРґРЅРѕРіРѕ СЃРѕС‚СЂСѓРґРЅРёРєР° РЅР°С€РµРіРѕ РҐРѕР»РґРёРЅРіР°! РћРіСЂРѕРјРЅРѕРµ СЃРїР°СЃРёР±Рѕ Р·Р° РЅР°СЃС‚СЂРѕРµРЅРёРµ, СЃР»Р°РґРєСѓСЋ Р¶РёР·РЅСЊ Рё РїСЂРµРІРѕСЃС…РѕРґРЅС‹Р№ РІРєСѓСЃ Рё РґРёР·Р°Р№РЅ Р’Р°С€РёС… С‚РѕСЂС‚РѕРІ!\r\n          </blockquote>\r\n        </div>\r\n      </div>\r\n    </div>\r\n  </section>'),
(6, 'feedback', '<section class=\"feedback\" id=\"feedback\">\r\n    <div class=\"content\">\r\n      <h2>Р“РћРўРћР’ Рљ Р—РђРљРђР—РЈ РР›Р РќР• РњРћР–Р•РЁР¬ РћРџР Р•Р”Р•Р›РРўР¬РЎРЇ? Р—Р’РћРќР!</h2>\r\n      <a href=\"tel:+72223332255\">\r\n        <h3>+7 (222) 333-22-55</h3>\r\n      </a>\r\n      <h4>РР›Р РћРЎРўРђР’Р¬ РќРђРњ РЎР’РћР™ РќРћРњР•Р  Р РњР« РџР•Р Р•Р—Р’РћРќРРњ РЎРђРњР</h4>\r\n\r\n\r\n      <form action=\"add_feedback.php\" method=\"post\" enctype=\"multipart/form-data\">\r\n        <div class=\"form__contact\">\r\n          <input type=\"text\" name=\"name\" placeholder=\"РќРђРџРРЁРРўР• РЎР’РћР• РРњРЇ\" required pattern=\"[A-Za-zРђ-РЇР°-СЏРЃС‘]{2,}\">\r\n          <input type=\"tel\" name=\"tel-number\" id=\"tel-number\" title=\" РЅР°РїСЂРёРјРµСЂ, 89041041706\"\r\n            placeholder=\"РќРћРњР•Р  РўР•Р›Р•Р¤РћРќРђ\" required pattern=\"[0-9]{11}\">\r\n        </div>\r\n\r\n        <div class=\"form__download\">\r\n          Р•РЎРўР¬ Р¤РћРўРћ РџР РРњР•Р Рђ?\r\n          <label for=\"download\">Р—РђР“Р РЈР–РђР™ Р—Р”Р•РЎР¬</label>\r\n          <input type=\"file\" id=\"download\" name=\"download\" accept=\".png, .jpg, .jpeg\">\r\n        </div>\r\n\r\n        <div class=\"form__submit\">\r\n          <input type=\"checkbox\" name=\"personal\" id=\"personal\">\r\n          <label for=\"personal\">\r\n            РЇ РЎРћР“Р›РђРЎР•Рќ РќРђ РћР‘Р РђР‘РћРўРљРЈ РџР•Р РЎРћРќРђР›Р¬РќР«РҐ Р”РђРќРќР«РҐ\r\n          </label>\r\n        </div>\r\n        <input type=\"submit\" value=\"РџРћРћР‘Р©РђРўР¬РЎРЇ\">\r\n      </form>\r\n    </div>\r\n  </section>'),
(7, 'map', '<section class=\"map\">\r\n    <div class=\"content\">\r\n      <h2>РњР« РќРђ РљРђР РўР•</h2>\r\n      <iframe\r\n        src=\"https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d1185.8234258591788!2d57.224571220779985!3d65.14509500553477!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x4477316d89272b07%3A0x40a2e859a9bf1d6c!2z0JHRgNC40YHRgtC-0LvRjA!5e0!3m2!1sru!2sru!4v1644919434959!5m2!1sru!2sru\"\r\n         style=\"border: 0\" allowfullscreen=\"\" loading=\"lazy\"></iframe>\r\n      <h2>Рі.РџРµС‡РѕСЂР°, СѓР». РџРµСЂРІРѕРјР°Р№СЃРєР°СЏ Рґ.25</h2>\r\n    </div>\r\n  </section>');
...
--
-- РРЅРґРµРєСЃС‹ С‚Р°Р±Р»РёС†С‹ `cakes`
--
ALTER TABLE `cakes`
  ADD PRIMARY KEY (`id_cake`);

--
-- РРЅРґРµРєСЃС‹ С‚Р°Р±Р»РёС†С‹ `feedback`
--
ALTER TABLE `feedback`
  ADD PRIMARY KEY (`id_feedback`);

--
-- РРЅРґРµРєСЃС‹ С‚Р°Р±Р»РёС†С‹ `fillings`
--
ALTER TABLE `fillings`
  ADD PRIMARY KEY (`id_filling`);

--
-- РРЅРґРµРєСЃС‹ С‚Р°Р±Р»РёС†С‹ `index_page`
--
...
--
-- AUTO_INCREMENT РґР»СЏ С‚Р°Р±Р»РёС†С‹ `cakes`
--
ALTER TABLE `cakes`
  MODIFY `id_cake` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;

--
-- AUTO_INCREMENT РґР»СЏ С‚Р°Р±Р»РёС†С‹ `feedback`
--
ALTER TABLE `feedback`
  MODIFY `id_feedback` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT РґР»СЏ С‚Р°Р±Р»РёС†С‹ `fillings`
--
ALTER TABLE `fillings`
  MODIFY `id_filling` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=13;

--
-- AUTO_INCREMENT РґР»СЏ С‚Р°Р±Р»РёС†С‹ `index_page`
--
ALTER TABLE `index_page`
...
('aerosmith', 'Aerosmith', 'РЎРЁРђ', 1970, 'С…Р°СЂРґ-СЂРѕРє'),
('pink-floyd', 'Pink Floyd', 'Р’РµР»РёРєРѕР±СЂРёС‚Р°РЅРёСЏ', 1965, 'РїСЃРёС…РѕРґРµР»РёС‡РµСЃРєРёР№-СЂРѕРє');

INSERT INTO `music_albums` (`group_id`, `title`, `release_year`, `release_country`, `cover_path`) VALUES
(2, 'The Dark Side of the Moon', 1973, 'Р’РµР»РёРєРѕР±СЂРёС‚Р°РЅРёСЏ', 'assets/covers/dark-side.jpg'),
(2, 'Wish You Were Here', 1975, 'Р’РµР»РёРєРѕР±СЂРёС‚Р°РЅРёСЏ', 'assets/covers/wish-you-were-here.jpg'),
(1, 'Greatest Hits', 1999, 'РЎРЁРђ', 'assets/covers/aerosmith-hits.jpg'),
(2, 'Abbey Road', 1969, 'Р’РµР»РёРєРѕР±СЂРёС‚Р°РЅРёСЏ', 'assets/covers/abbey-road.jpg');

INSERT INTO `music_tracks` (`album_id`, `title`, `note`) VALUES
(3, 'Back in the Saddle', 'РїРѕРґСЂРѕР±РЅРµРµ...'),
(3, 'Last Child', 'РїРѕРґСЂРѕР±РЅРµРµ...'),
(3, 'Rats in the Cellar', 'РїРѕРґСЂРѕР±РЅРµРµ...'),
(3, 'Combination', 'РїРѕРґСЂРѕР±РЅРµРµ...'),
(3, 'Sick As a Dog', 'РїРѕРґСЂРѕР±РЅРµРµ...'),
